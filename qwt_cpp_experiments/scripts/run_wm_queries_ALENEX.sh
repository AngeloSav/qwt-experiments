...
do
	...
	do
		string="./../build/perf_wavelet_tree_benchmark /workspace/<anonymized>/shared_data_sets/${text}.${size}"
		./runPerf.sh "${string}" i10pc138 noturbo >> ../experiments/ALENEX_WM_query_results.txt
	done
done
